...
mpd() {
    ...
    #icon f001
    if [ -z "$cur_song" ]; then
        echo "%{B${COLOR_DARKRED}} ${AC}urxvt -e zsh -c 'ncmpcpp' & sleep .1 && bspc window -t floating${AB}MPD: Stopped${AE} %{B-}"
    else
        ...
        cur_song="$cur_song"
        echo "%{B${COLOR_DARKRED}}${AC}urxvt -e zsh -c 'ncmpcpp' & sleep .1 && bspc window -t floating${AB} $cur_song  $prev $toggle $next ${AE}%{B-}"
    fi
}
...
lock() {
    command="lock.sh"
    echo ${AC}$command${AB}$(icon f023)${AE}
}
